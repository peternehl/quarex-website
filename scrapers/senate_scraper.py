"""
Quarex Senate Candidate Scraper (2026)
Scrapes Ballotpedia's master candidate list for all 2026 Senate races.

This uses the centralized "List of candidates" table on the main 2026 Senate
...
SENATE_2026_URL = "https://ballotpedia.org/United_States_Senate_elections,_2026"


def scrape_all_candidates():
    """
    Scrape all 2026 Senate candidates from Ballotpedia's master list.
    ...
    response = requests.get(SENATE_2026_URL, headers=headers, timeout=60)
    ...
    for table in candidates_panel.find_all('table', class_='widget-table'):
        ...
        # Extract state name from caption like "Alabama Senate Candidates - 2026"
        caption_text = caption.get_text(strip=True)
        match = re.match(r'([A-Za-z ]+) Senate Candidates', caption_text)
        if not match:
            continue
...
def build_library_json(all_candidates):
    ...
    library = {
        ...
        "description": "All 2026 Senate races with declared candidates. Auto-generated from Ballotpedia master list.",
        "last_updated": datetime.now().strftime("%Y-%m-%d"),
        "source": "Ballotpedia",
        "shelves": [
            {"name": "Class 2 Regular Elections", "books": []},
            {"name": "Special Elections", "books": []}
        ]
    }
    ...
    for state in sorted(all_candidates.keys()):
        ...
        # All regular 2026 races go to shelf 0
...
def run_scraper(auto_upload=False):
    """
    Run the Senate candidate scraper.

    Args:
        auto_upload: If True, upload results to server after scraping
    """
    print("\n" + "=" * 60)
    print("Quarex Senate Candidate Scraper (2026)")
    print("=" * 60)
    print(f"Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

    os.makedirs(config.LOCAL_BACKUP_DIR, exist_ok=True)
    output_file = os.path.join(config.LOCAL_CANDIDATE_DIR, "us-senate-2026-complete", "us_senate_2026_complete.json")
    ...
    # Print summary
    print(f"\nFound {len(all_candidates)} states:")
    total_r = 0
    total_d = 0
    total_i = 0
    ...
    if auto_upload:
        ...
        upload_single_file("us_senate_2026_complete.json")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Scrape 2026 Senate candidates from Ballotpedia")
