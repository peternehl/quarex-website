# US House of Representatives 2026 - Scraping & Update Process

This document describes the complete process for updating House candidate data from Ballotpedia. This is a custom workflow specific to the House library due to its unique structure (435 districts across 50 states).

## CRITICAL: How the House Library Works

The House library is MORE COMPLEX than Senate/Governors. Here's why:

**Senate/Governors structure:**
- One book file per state (e.g., `alabama.json`)
- Chapters = parties (Republican, Democratic)
- Topics = candidates

**House structure:**
- One book file per state (e.g., `alabama.json`)
- Chapters = districts (AL-01, AL-02, etc.)
- Topics = all candidates in that district (mixed parties)

**Key insight:** The app loads `alabama.json` when you click Alabama. That file must contain ALL districts and ALL candidates. The district subfolders (`alabama/al-01.json`) are NOT used by the app for navigation - they exist only as backup/reference.

## Required Files for the App to Work

1. **Library manifest:** `us-house-2026-complete/_manifest.json` - lists shelves
2. **Shelf manifest:** `2026-states/_manifest.json` - lists all 50 states as books
3. **State book files:** `2026-states/alabama.json`, etc. - contains districts as chapters with candidates as topics

If any of these are missing or malformed, the app will show errors.

## File Structure

```
libraries/politician-libraries/us-house-2026-complete/
├── _manifest.json                    # Library manifest (bookCount: 50)
└── 2026-states/                      # Shelf containing all states
    ├── _manifest.json                # Shelf manifest (lists 50 states)
    ├── alabama.json                  # STATE BOOK FILE (required for app!)
    ├── alaska.json
    ├── ... (50 state book files)
    ├── alabama/                      # State folder (backup/reference only)
    │   ├── _manifest.json            # District manifest
    │   ├── al-01.json                # District file (not used by app nav)
    │   └── ...
    └── ...
```

## State Book File Format (REQUIRED)

Each state book file (e.g., `alabama.json`) contains districts as chapters:
...
**Important notes:**
- Each chapter is a DISTRICT (not a party)
- Topics contain ALL candidates from ALL parties, mixed together
- Candidate names include party abbreviation: `"Name (R)"`, `"Name (D)"`, `"Name (I)"`
- This is different from Senate where chapters are parties

## District File Format (Backup/Reference)

The district files in subfolders (e.g., `alabama/al-01.json`) use party-based chapters:
...
```json
{
  ...
  "chapters": [
    {
      "name": "Republican",
      "topics": ["Jerry Carl (R)", "James Dees (R)"]
    },
    {
      "name": "Democratic",
      "topics": ["Clyde Jones (D)"]
    }
  ]
}
```

These files are NOT used for app navigation but serve as detailed backup data.
